    ...
    <aside className={`${widthClass} bg-white text-slate-800 flex flex-col h-screen sticky top-0 transition-all duration-300 ease-in-out z-20 flex-shrink-0 border-r border-[#F1ECE6] shadow-sm shadow-slate-900/5`}>
      ...
      <div className={`border-b border-[#F1ECE6] flex items-center overflow-hidden transition-all duration-300 ${isMounted && isCollapsed ? 'p-4 justify-center' : 'p-6 space-x-3'
        ...
        {isMounted && !isCollapsed && (
          <div className="flex items-center space-x-3 overflow-hidden flex-1 animate-fadeIn">
            <div className="bg-gradient-to-r from-[#2E5B9A] via-[#59C8CF] to-[#FFD7B5] p-2 rounded-lg text-slate-950 flex-shrink-0 ">
        ...
        {menuItems.map((item) => {
          ...
          return (
            <Link
              ...
              className={`flex items-center px-4 py-3 rounded-xl text-sm font-semibold transition-all ${isCollapsed ? 'justify-center' : 'space-x-3'
                } ${isActive
      ...
      <div className="p-4 border-t border-[#F1ECE6]">
        <Link
          ...
          className={`flex items-center rounded-xl text-sm font-semibold text-slate-600 hover:bg-slate-100 hover:text-slate-900 transition-all ${isCollapsed ? 'justify-center p-3' : 'justify-between px-4 py-3 border border-transparent hover:border-slate-200'
            }`}
        >
