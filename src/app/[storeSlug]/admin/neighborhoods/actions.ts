...
export async function updateNeighborhood(id: string, data: NeighborhoodInput, storeSlug: string) {
  if (!id) {
  ...
  revalidatePath(`/${storeSlug}/admin/neighborhoods`);
  revalidatePath(`/${storeSlug}`);
  return { success: true };
}

export async function updateMultipleNeighborhoods(ids: string[], data: Partial<NeighborhoodInput>, storeSlug: string) {
  if (!ids || ids.length === 0) {
